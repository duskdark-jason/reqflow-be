...
如果用户提供需求平台开发 Key，先通过需求平台 MCP 读取最终需求设计、任务分支、目标仓库、开发基线分支和验收要求，并按 `docs/process/platform-key-workflow.md` 的开发模式执行。必须校验当前 workspace 仓库远端与平台关联远端一致，并切换到需求设计阶段已经创建的任务分支；开发阶段不得重新生成不同任务分支。`developing` 阶段使用开发阶段 actionToken，先把阶段 actionToken、stage、targetMethod、demandNo、contentHash 和 lastContextAt 写入本地 `meta.md` 的 `platformSync`，再基于最终 `requirement.md` 判断是否适合拆分为多个 subagent 并行执行；只有职责边界清晰、无共享状态且可独立验证时才拆分，否则保持单执行路径。随后生成或更新 `plan.md`，再落地开发，本地完成后等待开发人员确认；只有用户明确确认提交验收或回写平台时，才依次回写执行计划、执行报告和 Review 报告。`repairing` 阶段使用返修阶段 actionToken，只处理返修项或需求人返修说明并补充 `execution-report.md` 和 `review-report.md`，不得重新生成需求设计或执行计划；用户明确确认提交返修验收后才回写平台。`closeout_pending` 阶段使用合并归档指令，先在每个目标仓库的任务分支完成 `docs/specs/active/REQ-*` 到 `docs/specs/done/` 的归档迁移，再 squash merge 任务分支到需求基线分支并 push，用户明确确认归档发布后，再调用 `publish_repository_index` 更新平台知识库并等待平台验证；平台验证通过后，才能删除本地开发分支并确认完成。
...
- 如果 Review Agent 产生 `RF-*`，自动切回执行阶段修复，按相同 RF ID 回填 `execution-report.md` 的 `Review 返修记录`，从本地 `meta.md` 的 `platformSync.actionToken` 恢复当前阶段 token，等用户明确确认提交返修验收或回写平台后，再通过 MCP `upload_execution_report` 回写新版本并交回 Review Agent 复审，直到最终 Review 通过。
...
- 列出每个仓库代码注释处理情况。
- 列出 companion 仓库是否同步。
- MCP 接入模式下说明是否已通过 MCP 回写执行报告新版本；如果尚未获得用户明确确认，写明“已完成本地迭代，等待用户确认回写平台”。本地 Harness 模式下写明“未接入 MCP，本地文件闭环”，不得伪造回写。
