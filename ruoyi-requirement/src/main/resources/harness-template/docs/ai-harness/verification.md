...
| L1 编译/构建 | 代码能通过静态编译或打包 | 大多数代码修改 | `【填写编译或构建命令】` |
| L2 单元/契约 | 核心逻辑、接口契约、组件契约可验证 | Service、utils、DTO/VO、组件状态 | `【填写专项测试命令】` |
| L3 运行态冒烟 | 应用能启动，接口/页面能真实运行 | Controller、页面、权限、配置 | `【填写本地启动和冒烟方式】` |
| L4 端到端/跨端 | 高价值用户流程可回归验证 | 可选：保存、导出、异步任务、跨服务联调 | `【填写端到端验证方式】` |
