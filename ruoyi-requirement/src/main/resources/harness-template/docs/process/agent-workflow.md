...
- MCP 接入模式下必须先输出需求可行性评估和风险判断，结论类型至少包含“可继续设计、需澄清、需调整、暂不可实现”；本地文件完成并经用户明确确认提交后，才通过 MCP `upload_requirement_assessment` 回写平台。结论需要需求人补充、调整或暂不可实现时，停止在反馈阶段。
...
- 本地 Harness 流程确需先写执行计划时，也必须把需求设计和执行计划分清，不得把用户选择方案直接当成执行授权。
...
- `plan.md` 未包含验收和验证前，不进入实现；MCP 接入模式和本地 Harness 模式下，`plan.md` 都由 Execution Agent 在执行阶段生成或更新。
...
- `planning` / `executing` / `repairing` / `complete` 在 MCP 接入模式下必须使用同一个任务分支；开发和返修阶段不得再创建不同任务分支，当前分支不得是 `main` 或 `master`。
...
- MCP 接入模式下，Plan Agent 在需求设计阶段先从目标基线分支创建 ASCII 任务分支；Execution Agent 只切换并沿用该分支，不再创建不同任务分支。不使用 worktree。
