...
如果用户提供需求平台需求设计 Key，先通过需求平台 MCP 读取项目、仓库远端、目标基线分支、建议任务分支、影响模块、历史需求设计版本和需求人补充调整指令，按 `docs/process/platform-key-workflow.md` 的需求设计模式执行。必须校验当前 workspace 仓库远端一致，切换到目标基线分支并 `git pull --ff-only`，再创建或切换到平台建议的 ASCII 任务分支。需求分析阶段只生成需求可行性评估和风险判断，把阶段 actionToken、stage、targetMethod、demandNo、contentHash 和 lastContextAt 写入本地 `meta.md` 的 `platformSync`；用户明确确认提交可行性评估后，才通过 MCP `upload_requirement_assessment` 使用需求分析 actionToken 回写平台。结论为需澄清、需调整或暂不可实现时，把结论反馈给需求人并停止，不生成 `requirement.md`。需求生成阶段使用新的需求生成 actionToken，只生成或调整 `requirement.md`，允许研发人员在本地多轮迭代和人工微调；研发人员明确确认定稿或提交需求设计后，才通过 MCP `save_requirement_package` 回写需求平台。不得生成 `plan.md`、不得改代码、不得写执行或 Review 报告。

如果没有需求平台 Key、未接入 MCP 或 MCP 工具不可用，按 `docs/process/local-harness-workflow.md` 使用本地 Harness 模式，把阶段文档写入本地 `docs/specs`。

用户选择方案、确认方向或同意建议，只代表允许你进入需求分析或需求生成阶段。你写完当前阶段文件后必须停止；需求分析评估等待用户确认是否回写平台，需求设计等待研发人员确认是否回写平台。完成真实 MCP 回写或本地文件落地后继续等待平台流转或明确执行授权。不得改业务代码、写 `plan.md`、写 `execution-report.md` 或写 `review-report.md`。需求人补充调整指令时，继续使用同一任务分支和同一 spec 目录更新最终 `requirement.md`；MCP 接入模式只有在研发人员明确确认后才再次回写形成新版本，本地 Harness 模式只记录本地文件闭环，不得伪造 MCP 回写。如发生上下文压缩或多轮对话，先从本地 `meta.md` 的 `platformSync.actionToken` 恢复阶段 token，再决定是否需要调用 `get_action_context` 读取最新轻量上下文。如需要重新评估风险，应回到需求分析阶段。
