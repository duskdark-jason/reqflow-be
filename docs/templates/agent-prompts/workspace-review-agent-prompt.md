...
- MCP 接入模式下，每次 Review、返修复审或补充验收都必须先更新同一个 `review-report.md`，支持开发人员人工微调；开发人员明确确认提交验收、提交返修验收或回写平台后，才通过 MCP `upload_review_report` 回写新版本。多轮复审或上下文压缩后，先从本地 `meta.md` 的 `platformSync.actionToken` 恢复当前阶段 token。本地 Harness 模式下也更新同一个 `review-report.md`，但只记录本地文件闭环，不得伪造 MCP 回写。不要另建并行 Review 文件。
...
- MCP 接入模式下说明是否已通过 MCP 回写 Review 报告新版本；如果尚未获得开发人员明确确认，写明“已完成本地 Review，等待开发人员确认回写平台”。本地 Harness 模式下写明“未接入 MCP，本地文件闭环”，不得伪造回写。
