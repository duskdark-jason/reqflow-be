...
- 初始化、需求设计和执行开发 actionToken 仅可使用一次，24 小时内有效，过期或已使用后需重新生成。
...
- 执行阶段：`develop-instruction` 同时给出执行计划、执行报告和 Review 报告三个 24 小时内有效、仅可使用一次的 actionToken，分别指向 `save_development_plan`、`upload_execution_report` 和 `upload_review_report`。
...
- AC-012：`develop-instruction` 指令文案包含执行计划、执行报告和 Review 报告三个目标工具，并分别使用一次性 actionToken 回写 `save_development_plan`、`upload_execution_report` 和 `upload_review_report`。
