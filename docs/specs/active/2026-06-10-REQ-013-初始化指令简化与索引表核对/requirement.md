...
- 如果本地平台库确实缺少索引表，按既有 `docs/db/sql/req_platform_req007_index_tables.sql` 补齐本地库。
...
- AC-BE-003：现有 `publish_repository_index` 缺索引表路径继续返回 `平台索引表未初始化`，并指向 `docs/db/sql/req_platform_req007_index_tables.sql`。
...
- 数据库/SQL：不新增 SQL 文件；本地库可能执行既有 `docs/db/sql/req_platform_req007_index_tables.sql`。
- 权限：无影响。
- 页面展示：文案长度变短，页面结构不变。
