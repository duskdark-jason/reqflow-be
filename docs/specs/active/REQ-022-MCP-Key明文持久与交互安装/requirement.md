...
- MCP 管理页仅管理员展示请求地址配置栏；开发人员不展示配置栏，仍可管理自己有权限的 MCP Key。
...
- AC-011：MCP 管理页仅管理员展示 MCP 请求地址配置栏，保存 `publicHost` 后展示后端返回的完整地址，普通开发人员不可配置。
...
- 页面展示：是，MCP 管理页隐藏明文 Key 和 Key 前缀字段，统一命令执行后选择工具；管理员额外看到 MCP 请求地址配置栏。
