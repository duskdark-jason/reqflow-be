...
| AC-002 | MCP 配置或命令覆盖客户端，含 OpenCode remote | `clientInstructions` | 后端目标测试 | 通过 |
| AC-003 | 全局 skill 使用 `npx skills add` 覆盖各客户端 | `skillInstall.commands` | 后端目标测试 | 通过 |
| AC-004 | 不泄漏明文 Key 或 actionToken | 模板占位符和测试断言 | 后端目标测试 | 通过 |
| AC-005 | 前端分组展示安装指令 | `mcpKey/index.vue` | `npm run build:prod` | 通过 |
