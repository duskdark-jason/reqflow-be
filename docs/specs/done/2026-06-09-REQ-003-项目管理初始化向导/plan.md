...
- Companion：`../../../../reqflow-ui/docs/specs/done/2026-06-09-REQ-003-项目管理初始化向导`
- 目标客户与基线分支：通用 / main

## 实施步骤

1. 初始化 DTO：新增项目初始化请求、响应、仓库项、分支项、模块摘要、索引摘要和检查项 DTO，覆盖 AC-BE-001、AC-BE-002。
2. 初始化 Service：新增 `IReqProjectInitService` 和实现类，聚合读取项目、仓库、分支配置、模块数量和索引批次摘要，覆盖 AC-BE-001。
3. 聚合保存事务：实现新增和更新初始化方法，在同一事务内保存项目、仓库和分支配置；任一校验失败整体回滚，覆盖 AC-BE-002、AC-BE-003。
4. 参数和路径校验：复用或抽取 REQ-002 的个人绝对路径校验规则，校验 Git 远端、默认分支、真实分支名和备注，覆盖 AC-BE-004。
...
6. 测试：增加 Service 单元测试，覆盖成功保存、保存失败回滚、个人路径拒绝、初始化摘要聚合、分支中文标签回显、`variantCode` 兜底生成和权限边界；覆盖 AC-BE-001 至 AC-BE-005。
...
| 新增 | `ruoyi-requirement/src/main/java/com/ruoyi/requirement/dto/ReqProjectInitVariantItem.java` | 初始化分支项，兼容既有客户定制线字段 |
...
| AC-BE-002 | 聚合保存事务、分支项兼容 | Service 测试和跨端联调 |
...
- 不保存个人本机绝对路径；后端只保存 Git 远端、真实分支名、需求人员展示中文标签、commit、相对路径和结构化初始化数据。
