...
未发现阻断或重要问题。当前实现满足后端验收要求，可以按用户授权进入自动办结收口。
...
- Key 服务与数据访问：`ReqMcpUserKey`、`ReqMcpUserKeyCreateResult`、`ReqMcpUserKeyMapper`、`IReqMcpUserKeyService`、`ReqMcpUserKeyServiceImpl`、`ReqMcpUserKeyMapper.xml`
...
- L3/L4 真实后端联调未执行，尚未在真实数据库中执行菜单 SQL，也未通过登录态和 `X-MCP-Key` 调真实 `/requirement/mcp` 接口。该风险已在 `execution-report.md` 记录，不影响当前代码静态审查结论。
- `req:mcp:key:query` 与 `req:mcp:key:edit` 分离，实际角色配置时应同时给维护人员分配查询权限，否则编辑弹窗读取详情会被后端拒绝；当前菜单 SQL 已创建查询权限点，属于角色配置注意事项。

## 验收覆盖

- AC-BE-001：通过。菜单 SQL 已新增 MCP 管理菜单和 `req:mcp:key:list/query/add/edit/remove` 权限点；管理接口均使用 RuoYi 权限注解。
- AC-BE-002：通过。服务生成 `reqflow_mcp_` 随机 Key，落库 `key_hash` 与 `key_prefix`，明文只在 `ReqMcpUserKeyCreateResult` 返回；测试覆盖明文不入库。
- AC-BE-003：通过。表设计允许同一 `user_id` 多 Key；停用 Key 在 `authenticate` 拒绝，删除接口删除 Key。
...
- `mvn -pl ruoyi-requirement -am -Dtest=ReqMcpUserKeyServiceImplTest -Dsurefire.failIfNoSpecifiedTests=false test`：5 tests 通过。
- `mvn -pl ruoyi-requirement -am test`：39 tests 通过。
- `mvn -pl ruoyi-admin -am -DskipTests package`：通过。
- `sh scripts/check-docs.sh`：通过。

未执行真实后端服务启动、数据库迁移和 MCP HTTP 联调，建议在部署或本地联调阶段补验。
