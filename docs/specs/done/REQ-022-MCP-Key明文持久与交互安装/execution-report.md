...
| `ReqActionTokenServiceImpl.java`、`ReqDemandServiceImpl.java`、`ReqRepositoryIndexServiceImpl.java`、`McpService.java`、`ReqflowCodexGlobalSkillTemplate.java` | 将需求分析、需求设计、开发、返修、归档统一为阶段内可复用 actionToken；归档改为单需求级 token 多仓复用；`get_action_context` 返回 `tokenPersistence` 和 `writebackPolicy`，全局 skill 要求本地迭代后等待用户确认再回写平台。 |
| `docs/process/**`、`docs/templates/agent-prompts/**`、`ruoyi-requirement/src/main/resources/harness-template/docs/**` | 同步平台 Key 流程、Agent 交接流程、Workspace Agent prompt 和项目初始化下发模板，要求多轮迭代或上下文压缩后从本地 `meta.md platformSync.actionToken` 恢复 token，且写平台工具必须等用户明确确认。 |
...
- 影响模块：MCP 管理、MCP 请求地址配置、MCP Key 持久化、多客户端安装脚本、本地 Harness 门禁、合并归档指令、需求返修流程、返修 MCP 报告回写、阶段短指令、轻量上下文与全局 skill、阶段 token 控制、平台回写确认门禁、actionToken 本地持久化
- 模块知识库动作：更新
...
| L2 | AC-019、AC-020、AC-021 | `mvn -pl ruoyi-requirement -am -Dtest=ReqActionTokenServiceImplTest,ReqDemandServiceImplTest,ReqRepositoryIndexServiceImplTest,McpServiceTest,ReqflowCodexGlobalSkillTemplateTest -Dsurefire.failIfNoSpecifiedTests=false test` | 通过，先红于阶段 token 仍一次性、归档仍多 token、skill 缺少确认门禁和 token 持久化，修复后 106 个测试通过 |
| L1 | AC-005 | `npm run build:prod`（companion 前端） | 通过，存在历史体积告警 |
| L0 | AC-006、AC-007、AC-008、AC-009、AC-010、AC-011、AC-013、AC-014、AC-015、AC-016、AC-017、AC-018、AC-019、AC-020、AC-021 | `sh scripts/check-docs.sh && sh scripts/check-harness.sh complete --spec docs/specs/active/REQ-022-MCP-Key明文持久与交互安装` | 通过 |
...
- 用户补充需求设计和开发阶段不能自动推送平台，必须允许本地多轮迭代并由用户明确确认后再提交；已在后端上下文、全局 skill、流程文档和初始化模板中增加平台回写确认门禁。
- 用户补充多轮对话或上下文压缩后不能丢失 actionToken；已要求执行端把阶段 actionToken 写入本地 `meta.md platformSync.actionToken`，并在恢复时优先读取本地 token 后再按 hash 判断是否拉取平台全文。
