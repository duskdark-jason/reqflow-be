...
- `req_action_token.expire_time` 和 `last_used_time` 是安全边界字段：动作 token 以需求流程阶段为有效边界，流转到下一流程即失效；`expire_time` 作为最长 24 小时兜底，不能作为长期项目、分支或需求缓存键。项目初始化 token 一次性消费；需求分析、需求生成、开发、返修和合并归档 token 在当前阶段内可重复用于对应写回或索引发布并刷新 `last_used_time`。
