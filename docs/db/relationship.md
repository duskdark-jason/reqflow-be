...
| `req_variant` | 项目下的客户定制线、主线范围或分支配置 | 一行一个可展示分支配置 |
...
- 在 REQ-003 项目维护弹窗中，`req_variant.variant_name` 兼容承载需求人员可见中文标签，`req_variant.baseline_branch` 承载真实 Git 分支名；`variant_code` 保持唯一键需要，允许由后端根据真实分支名兜底生成。
