...
- 指定开发人员可通过需求详情获取 `requirement_plan` 动作 token 指令；`submitted` 状态生成需求分析 actionToken，`plan_pending` 状态生成需求生成 actionToken，`plan_ready` 为需求设计待确认阶段，不生成需求设计指令。平台复制指令只保留 `reqflow-mcp`、`mcpServer: reqflow`、actionToken 和先调用 `get_action_context` 的提醒，不再列出 `stage`、`targetMethod`、需求编号、分支、具体 `mcpTool` 或阶段执行手册；具体工具映射由全局 skill 先读取轻量上下文后识别：`requirement_analysis -> upload_requirement_assessment`，`requirement_generate -> save_requirement_package`。需求分析阶段默认只生成本地评估草稿，必须等用户明确确认后才回写平台；需求设计阶段默认只生成本地设计草稿，支持研发人员人工微调，必须等研发人员明确确认后才回写平台。
- 指定开发人员只能在点击“开始开发”进入 `developing` 后，通过需求详情获取开发阶段 actionToken；`confirmed` 待执行开发阶段不展示也不生成执行指令。该指令只给出一个开发阶段 actionToken，不展开执行计划、执行报告和 Review 报告的回写顺序；全局 skill 先调用 `get_action_context` 再映射到 `save_development_plan`、`upload_execution_report` 和 `upload_review_report`，并继续约束生成 `plan.md` 前的 subagent 拆分判断。开发阶段默认只做本地计划、开发、验证和报告草稿，支持开发人员人工微调和初步验证，必须等开发人员明确确认提交验收或回写平台后才调用写平台工具。
- 指定开发人员可在 `repairing` 状态通过需求详情获取返修阶段 actionToken；该指令只给出一个返修阶段 actionToken，不展开返修步骤。全局 skill 先调用 `get_action_context` 再映射到 `upload_execution_report` 和 `upload_review_report`，并约束读取 Review 报告和需求人返修问题说明、不得重新生成需求设计或执行计划。返修阶段默认只做本地修复、验证和报告草稿，支持开发人员人工微调，必须等开发人员明确确认提交返修验收后才回写平台。返修阶段报告上传支持两种安全形态：上传完整本地 `execution-report.md` 或 `review-report.md` 时按完整内容保存新版本；只上传本轮返修片段时，服务端基于上一版报告追加“返修执行记录”或“返修 Review 记录”后保存新版本，最新版报告不得只剩返修片段。
...
- 项目初始化动作 token 生成后 24 小时内有效，在当前项目分支初始化动作内可复用，支持同一项目分支的前端、后端等多个仓库连续发布索引；需求分析、需求生成、开发、返修和合并归档动作 token 在当前流程阶段内可复用，最长保留 24 小时，需求流转到下一阶段后旧 token 立即失效。
...
- 需求分析和需求生成指令调整时，必须保持阶段收敛：平台指令只给 actionToken，`get_action_context` 返回允许工具；需求分析阶段只允许 `upload_requirement_assessment`，需求生成阶段只允许 `save_requirement_package`。结论允许继续后，才在需求生成阶段落地 `requirement.md`；需求设计版本回写必须等待研发人员明确确认。开发阶段只能沿用该分支生成 `plan.md` 和实现。
- 返修指令调整时，必须保持同一任务分支和同一 spec 目录，平台指令只给同一个返修阶段 actionToken，`get_action_context` 返回 `upload_execution_report`、`upload_review_report` 允许工具，持续追加本地 `execution-report.md` 与 `review-report.md`，不得携带执行计划或需求设计生成要求；平台回写必须等待开发人员明确确认。
