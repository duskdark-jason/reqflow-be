...
- MCP 管理配置或 Key 创建结果调整时，必须同步前端 MCP 管理页。全局 Codex skill 应通过跨平台 `codexGlobalSkillPackage` 返回，由 Codex 按自身 skill 规范安装，不能只下发 macOS/Linux shell 命令；推荐同时返回 `codexSetupPackage`，把 MCP 配置、全局 skill、短提示词和 server metadata 组合成一次复制入口，但不得在配置后自动调用 reqflow MCP tools。
