...
- 指定开发人员可通过需求详情获取 `requirement_plan` 动作 token 指令；`submitted` 状态生成 `stage=requirement_analysis` 短指令，`plan_pending` 状态生成 `stage=requirement_generate` 短指令，`plan_ready` 为需求设计待确认阶段，不生成需求设计指令。平台复制指令只保留 `reqflow-mcp`、`mcpServer: reqflow`、`stage`、`targetMethod`、需求编号、任务分支、actionToken 和 `arguments.actionToken` 提醒，不再列出具体 `mcpTool` 或阶段执行手册；具体工具映射由全局 skill 识别：`requirement_analysis -> upload_requirement_assessment`，`requirement_generate -> save_requirement_package`。
- 指定开发人员只能在点击“开始开发”进入 `developing` 后，通过需求详情获取 `stage=requirement_develop` 开发阶段短指令；`confirmed` 待执行开发阶段不展示也不生成执行指令。该指令只给出一个开发阶段 actionToken 和动态上下文，不展开执行计划、执行报告和 Review 报告的回写顺序；全局 skill 负责映射到 `save_development_plan`、`upload_execution_report` 和 `upload_review_report`，并继续约束生成 `plan.md` 前的 subagent 拆分判断。
- 指定开发人员可在 `repairing` 状态通过需求详情获取 `stage=requirement_repair` 返修阶段短指令；该指令只给出一个返修阶段 actionToken 和动态上下文，不展开返修步骤。全局 skill 负责映射到 `upload_execution_report` 和 `upload_review_report`，并约束读取 Review 报告和需求人返修问题说明、不得重新生成需求设计或执行计划。返修阶段报告上传支持两种安全形态：上传完整本地 `execution-report.md` 或 `review-report.md` 时按完整内容保存新版本；只上传本轮返修片段时，服务端基于上一版报告追加“返修执行记录”或“返修 Review 记录”后保存新版本，最新版报告不得只剩返修片段。
- 指定开发人员可在 `closeout_pending` 状态通过需求详情获取 `stage=requirement_closeout` 合并归档短指令；该指令按项目分支下每个有效仓库生成一次性 `publish_repository_index` actionToken 和仓库清单，不再展开 active->done、squash merge、push、publish、删除分支等步骤。完整归档顺序由全局 skill 承接。需求流转到 `completed` 前，服务端必须逐仓校验本需求生成的合并归档 token 已被使用，且对应仓库产生带本需求归档上下文的 imported 索引批次；`/requirement/demand/{demandId}/closeout-verification` 复用同一验证口径返回只读验证结果，供前端在生成合并归档指令和确认归档完成之间互斥展示。
...
- 全局 `reqflow-mcp` skill 模板的 `SKILL.md` frontmatter 必须保持合法 YAML；`name` 和 `description` 使用双引号包裹，描述中不得出现未转义的 `: `，避免 Codex、Claude Code、Trae、Qoder、CodeBuddy 或 OpenCode 启动扫描时跳过该 skill。skill 正文必须维护阶段工具映射表，让短指令中的 `stage` 直接映射到允许调用的 MCP tools、本地文件边界和停止条件。
