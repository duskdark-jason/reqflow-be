...
`plan-instruction` 接口返回 `ReqActionInstruction`，`actionType=requirement_plan`，仅指定开发人员或管理员可在 `submitted` 或 `plan_pending` 状态生成。平台复制内容必须是短动态上下文，只保留 `reqflow-mcp`、`mcpServer: reqflow`、`stage`、`targetMethod`、`demandId`、`demandNo`、任务分支、对应阶段 `actionToken` 和 `arguments.actionToken` 使用提醒，不在指令正文列出具体 `mcpTool` 或完整执行步骤。`submitted` 返回 `stage=requirement_analysis`、`targetMethod=requirement_analysis` 和需求分析 actionToken；`plan_pending` 返回 `stage=requirement_generate`、`targetMethod=requirement_generate` 和需求生成 actionToken；`plan_ready` 为需求设计待确认阶段，不生成需求设计指令。具体 MCP 工具选择、读取资源、本地文件边界和停止条件由全局 `reqflow-mcp` skill 根据 `stage` 识别。

`develop-instruction` 接口返回 `ReqActionInstruction`。`developing` 和 `repairing` 状态返回 `actionType=requirement_develop`，`closeout_pending` 状态返回 `actionType=requirement_closeout`；仅指定开发人员或管理员可生成。`confirmed` 待执行开发阶段只允许指定开发人员点击“开始开发”流转到 `developing`，不生成执行指令。`developing` 返回 `stage=requirement_develop`、`targetMethod=requirement_develop` 和开发阶段 actionToken；`repairing` 返回 `stage=requirement_repair`、`targetMethod=requirement_repair` 和返修阶段 actionToken；`closeout_pending` 返回 `stage=requirement_closeout`、`targetMethod=publish_repository_index`，并按目标仓库列出 repo 信息和一次性归档 actionToken。复制内容不展开 `save_development_plan`、`upload_execution_report`、`upload_review_report`、`publish_repository_index` 的调用顺序，也不重复分支、Review、返修或归档步骤。全局 `reqflow-mcp` skill 必须维护 `stage -> MCP tool` 映射：`requirement_develop -> save_development_plan, upload_execution_report, upload_review_report`，`requirement_repair -> upload_execution_report, upload_review_report`，`requirement_closeout -> publish_repository_index`。开发阶段、返修阶段和归档阶段 actionToken 都不能替代人员 `X-MCP-Key`。
