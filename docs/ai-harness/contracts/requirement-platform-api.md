...
`plan-instruction` 接口返回 `ReqActionInstruction`，`actionType=requirement_plan`，仅指定开发人员或管理员可在 `submitted` 或 `plan_pending` 状态生成。平台复制内容必须是最小动态上下文，只保留 `reqflow-mcp`、`mcpServer: reqflow`、对应阶段 `actionToken` 和先调用 `get_action_context` 的提醒，不在指令正文列出 `stage`、`targetMethod`、需求编号、分支、具体 `mcpTool` 或完整执行步骤。`submitted` 生成需求分析 actionToken，`plan_pending` 生成需求生成 actionToken，`plan_ready` 为需求设计待确认阶段，不生成需求设计指令。具体阶段、MCP 工具选择、读取资源、本地文件边界和停止条件由全局 `reqflow-mcp` skill 先调用 `get_action_context` 后根据返回 `stage` 识别。

`develop-instruction` 接口返回 `ReqActionInstruction`。`developing` 和 `repairing` 状态返回 `actionType=requirement_develop`，`closeout_pending` 状态返回 `actionType=requirement_closeout`；仅指定开发人员或管理员可生成。`confirmed` 待执行开发阶段只允许指定开发人员点击“开始开发”流转到 `developing`，不生成执行指令。`developing` 返回开发阶段 actionToken；`repairing` 返回返修阶段 actionToken；`closeout_pending` 返回一个需求级合并归档 actionToken，所有仓库发布索引都复用该 token，复制内容不列出仓库名称、repoId、分支或 `targetMethod`。复制内容不展开 `save_development_plan`、`upload_execution_report`、`upload_review_report`、`publish_repository_index` 的调用顺序，也不重复分支、Review、返修或归档步骤。全局 `reqflow-mcp` skill 必须先调用 `get_action_context`，再维护 `stage -> MCP tool` 映射：`requirement_develop -> save_development_plan, upload_execution_report, upload_review_report`，`requirement_repair -> upload_execution_report, upload_review_report`，`requirement_closeout -> publish_repository_index`。开发阶段、返修阶段和归档阶段 actionToken 都不能替代人员 `X-MCP-Key`。

`closeout_pending` 的详细执行顺序由全局 `reqflow-mcp` skill 承接：先在任务分支完成 active spec 到 done 的归档迁移，再 squash merge 到需求基线分支、push。用户明确确认归档发布后，才按当前完整快照调用 `publish_repository_index` 更新知识库，平台验证通过后再删除本地开发分支。服务端按需求生成一个合并归档 actionToken；该 token 只用于当前需求当前阶段的 `publish_repository_index`，最长 24 小时且阶段内可复用，各仓库通过 `remoteUrl` 定位。`closeout-verification` 返回 `{ "verified": true|false, "message": "..." }`，复用 `closeout_pending -> completed` 的逐仓验证口径，只读查询不更新需求状态；未通过时 `verified=false` 且 `message` 包含未通过原因。需求从 `closeout_pending` 流转到 `completed` 前，服务端必须逐仓确认本需求对应仓库都已产生带本需求归档上下文的 `imported` 索引批次；否则返回 `归档结果未通过平台验证` 类业务异常。
...
执行包保存永远追加 `req_package_version` 新记录，不覆盖历史版本。版本号按 `demand_id + artifact_type` 独立递增。需求设计阶段先追加需求可行性评估版本，再按结论追加需求设计版本。验收返修不新建需求，继续在同一需求下追加需求设计、执行计划、执行报告或 Review 报告版本，用版本链表达返修轮次。返修阶段 MCP `upload_execution_report` 和 `upload_review_report` 额外保证最新版仍保留原报告正文：如果上传内容已经包含上一版正文，按上传完整内容保存；如果上传内容只是返修片段，服务端把片段追加到上一版之后再保存新版本。

执行包读取复用需求参与人可见性：需求创建人、指定开发人员和管理员可读。执行包保存、草稿生成和 MCP 资料包回写仅允许指定开发人员或管理员执行；即使用户拥有隐藏 `req:package:save` 权限，也不能回写未指定给自己的需求。服务端内部在提需时自动生成 `requirement_draft` 和 `context_manifest`，在需求人补充说明、需求设计调整或返修问题说明时生成 `requirement_supplement`，这些流程写入不代表开放通用资料包写权限。

业务页面默认一级标签展示的资料类型为 `requirement_draft`、`requirement_assessment`、`requirement`、`plan`、`execution_report` 和 `review_report`。`requirement_supplement` 仍作为补充/调整/返修问题版本类型和 MCP 资源保留，但前端应按语义嵌入需求可行性评估、需求设计或 Review 报告标签内的折叠历史记录，不作为普通页面默认标签展示；`context_manifest` 仍作为 MCP 上下文资源保留，不作为普通页面默认标签展示。

MCP `upload_requirement_assessment` 和 `save_requirement_package` 可显式传 `demandId`，也可传当前阶段指令里的对应 `actionToken` 由服务端解析到绑定需求；MCP `save_development_plan`、`upload_execution_report` 和 `upload_review_report` 可传开发执行或返修指令里的同一个阶段 `actionToken` 由服务端解析到绑定需求；这些调用仍必须通过人员 `X-MCP-Key` 或登录态权限校验。需求可行性分析、需求设计、开发、返修和归档 actionToken 都是阶段内可复用 token：成功解析后刷新 `last_used_time`，但不立即作废；需求流转到下一阶段后旧 actionToken 立即失效，最长保留时间仍为 24 小时。返修阶段 `upload_review_report` 会在资料包版本保存后自动触发返修验收提交，因此全局 skill 必须要求开发人员明确确认提交返修验收后才能调用。

MCP `get_action_context` 是只读轻量上下文工具，入参为 `{ "actionToken": "..." }`，不会消费 actionToken，也不会更新 `last_used_time`。返回内容包含 `stage`、`actionType`、`targetMethod`、`allowedTools`、需求/项目分支/仓库摘要、`resources`、`packageVersions`、`syncPolicy`、`tokenPersistence` 和 `writebackPolicy`；合并归档 token 额外返回 `repositories`。`packageVersions` 只返回 `artifactType`、`versionNo`、`versionNote`、更新时间、`contentHash` 和 `resourceUri`，不得直接返回资料包正文。执行端必须先用该工具识别阶段，再将本地 `meta.md platformSync` 与 `packageVersions` 比对：版本和哈希一致时使用本地文件；本地缺失或不一致时才按 `resourceUri` 读取全文。执行端还必须把 actionToken 写入本地 `meta.md platformSync.actionToken`，用于多轮本地迭代或上下文压缩后的恢复；写平台工具必须等待用户明确确认后再调用。需求资源模板新增 `requirement://{demandNo}/package/{artifactType}`，用于按产物类型读取最新资料包全文。
...
| `/requirement/mcp/key/config` | GET | `admin` 角色 | 管理员查询 MCP 请求地址配置，返回 `configKey`、`publicHost` 和服务端拼接后的完整 `mcpAddress` |
| `/requirement/mcp/key/config` | PUT | `admin` 角色 | 管理员保存 MCP 请求地址配置；`publicHost` 只能填写域名/IP和端口，不得填写协议、路径、查询串或空白字符 |
...
`/requirement/mcp/key/config` 只服务于 MCP 管理页的管理员配置弹窗。普通开发人员不展示配置入口，页面仍不得常驻展示请求头名 `X-MCP-Key`、客户端配置模板、全局 skill 包或安装指令包。

创建 Key 和打开指令响应均返回 `key`、`plainKey` 和 `codexSetupPackage`。其中 `plainKey` 由前端用于把明文渲染进可复制安装命令，页面下次打开使用指令时也必须从顶层 `plainKey` 或 `key.plainKey` 读取真实明文并替换占位符，但页面不得单独展示明文 Key 字段；`codexSetupPackage` 是多客户端安装指令包，`packageName=reqflow-mcp-multi-client-setup`，`installScope=global`，并包含 `supportedClients`、`mcpServer`、`codexConfigTemplate`、`installScripts`、`installCommands`、`clientInstructions`、`skillPackage`、`installPrompt` 和 `serverMetadata`。`supportedClients` 只包含 `codex`、`claude-code`、`trae`、`qoder`、`codebuddy`、`opencode`。`installCommands[]` 是前端主展示入口，必须只给出一组统一安装指令；用户执行后由脚本交互选择 Codex、Claude Code、Trae、Qoder、CodeBuddy、OpenCode 或全部工具，自动化场景仍可通过 `--client all|codex|claude-code|trae|qoder|codebuddy|opencode` 或 `-Client` 跳过交互。脚本输出必须区分 `Reqflow automatic MCP configuration completed` 和 `Manual MCP import required`，不能把仅生成 JSON 片段的客户端描述为已完成 MCP 安装。`clientInstructions[]` 仅作为高级配置/调试信息的一部分保留，每项包含 `client`、`label`、`transport`、`mcpConfigPath`、`mcpConfigSnippet`、`commands[]`、`skillInstall` 和 `notes`；前端普通弹窗不得按客户端拆开展示。`skillInstall.commands[]` 是单独安装全局 skill 的备用入口，继续使用 `npx skills add`，分别定向 `-a codex`、`-a claude-code`、`-a trae`、`-a qoder`、`-a codebuddy`、`-a opencode`。通用安装脚本在执行 `npx skills add` 后还必须强制覆盖所选工具的已知全局 skill 目录，重复安装必须刷新旧版 `reqflow-mcp/SKILL.md`：共享目录 `${AGENTS_HOME:-$HOME/.agents}/skills/reqflow-mcp/SKILL.md`，以及 Codex `${CODEX_HOME:-$HOME/.codex}/skills/reqflow-mcp/SKILL.md`、Claude Code `${CLAUDE_HOME:-$HOME/.claude}/skills/reqflow-mcp/SKILL.md`、Trae `${TRAE_HOME:-$HOME/.trae}/skills/reqflow-mcp/SKILL.md`、Qoder `${QODER_HOME:-$HOME/.qoder}/skills/reqflow-mcp/SKILL.md`、CodeBuddy `${CODEBUDDY_HOME:-$HOME/.codebuddy}/skills/reqflow-mcp/SKILL.md`、OpenCode `${OPENCODE_HOME:-${OPENCODE_CONFIG_DIR:-$HOME/.config/opencode}}/skills/reqflow-mcp/SKILL.md`。MCP 命令或配置中使用 `${REQFLOW_MCP_KEY}` 作为 Key 占位符，后端模板不得直接包含人员明文 Key 或一次性 `actionToken`，前端只在安装命令渲染时用响应明文替换占位符。OpenCode 的 MCP 配置使用 `opencode.json` 的 `mcp.reqflow`，`type=remote`，并通过 `headers` 携带 `X-MCP-Key`；已有 OpenCode JSON 配置必须优先自动合并，无法解析时才输出手工合并片段。`mcpServer` 必须包含 `name=reqflow`、`transport=streamable-http`、`url` 和 `headerName=X-MCP-Key`；`serverMetadata` 参考 MCP registry/server.json 风格，描述远程 MCP 地址、鉴权 header、`project-init`、`index-publish`、`package-handoff` 工具组和安全提示。配置完成后不得自动调用 `publish_repository_index` 或其他工具。

安装脚本端点：

| 路径 | 方法 | 权限 | 说明 |
|---|---|---|---|
| `/requirement/codex/install.sh` | GET | 匿名可读 | 返回 macOS/Linux 通用安装脚本，脚本从 `REQFLOW_MCP_KEY` 或 `--key` 读取人员 Key；不传 `--client` 时在执行后交互选择工具，传 `--client all|codex|claude-code|trae|qoder|codebuddy|opencode` 时执行全部或指定客户端安装流程；Codex、可用 CLI 的 Claude Code/CodeBuddy、可解析 JSON 的 CodeBuddy/OpenCode 会写入或合并配置，Trae/Qoder 以及无法自动合并的场景必须输出 `Manual MCP import required`；脚本调用 `npx skills add -g -a <client> --copy -y` 安装全局 `reqflow-mcp` skill，并强制覆盖所选客户端的已知全局 skill 目录 |
| `/requirement/codex/install.ps1` | GET | 匿名可读 | 返回 Windows PowerShell 通用安装脚本，脚本从 `REQFLOW_MCP_KEY` 或 `-McpKey` 读取人员 Key；不传 `-Client` 时在执行后交互选择工具，传 `-Client all|codex|claude-code|trae|qoder|codebuddy|opencode` 时执行全部或指定客户端安装流程；Codex、可用 CLI 的 Claude Code/CodeBuddy、可解析 JSON 的 CodeBuddy/OpenCode 会写入或合并配置，Trae/Qoder 以及无法自动合并的场景必须输出 `Manual MCP import required`；脚本调用 `npx skills add -g -a <client> --copy -y` 安装全局 `reqflow-mcp` skill，并强制覆盖所选客户端的已知全局 skill 目录 |
...
安装脚本和 skill 内容端点不得内置人员明文 Key，不得自动调用 reqflow MCP tool；脚本执行后只提示用户重启或刷新目标客户端。Codex 脚本可直接写 `~/.codex/config.toml`；Claude Code 和 CodeBuddy 优先调用各自 CLI 写入配置；OpenCode 必须写入或合并 `~/.config/opencode/opencode.json`（存在 `opencode.jsonc` 且无法自动解析时输出片段），CodeBuddy CLI 不可用时按官方优先级写入或合并 `~/.codebuddy/.mcp.json`、`~/.codebuddy/mcp.json` 或 `~/.codebuddy.json`；Trae、Qoder 按官方设置页 JSON 导入路径输出 `mcpServers` 片段并列入 `Manual MCP import required`。无论 MCP 配置是否需要手工导入，所选客户端的全局 skill 都必须由脚本覆盖刷新，确保平台更新 skill 后再次执行安装指令即可拿到最新版本。
...
入口：`POST /requirement/mcp`，发布环境完整路径为 `POST /reqflow-api/requirement/mcp`。该路径允许匿名进入安全过滤链，Controller 内部按两种方式鉴权：
...
- `publish_repository_index` 无 actionToken 时必须校验 `req:index:import`；携带项目初始化或需求合并归档 actionToken 时，可跳过宽泛索引导入权限，由索引服务校验 token 绑定项目、分支、需求状态和有效期，并用 `remoteUrl` 在同项目下定位仓库。合并归档 token 是需求级阶段 token，发布成功后索引批次必须记录本需求归档上下文，供 `closeout_pending -> completed` 逐仓校验。该工具优先接收 `actionToken + remoteUrl`，兼容旧 `mcpKey + remoteUrl`，只写入索引批次、模块知识、影响面条目和活动日志；上传内容不得包含个人本机绝对路径。
- 报告上传、计划保存和执行资料类工具必须校验 `req:package:save`，并且只追加 `req_package_version`。`upload_requirement_assessment` 支持需求分析 actionToken 定位需求；`save_requirement_package` 支持需求生成 actionToken 定位需求；`save_development_plan`、`upload_execution_report` 和 `upload_review_report` 支持开发阶段 actionToken 定位需求；返修阶段只支持 `upload_execution_report` 和 `upload_review_report` 使用同一个返修阶段 actionToken 定位需求，片段上传时服务端基于上一版报告追加返修记录，完整报告上传时保存完整报告。所有写平台工具都应由全局 skill 按阶段等待确认后调用：需求分析等待用户确认，需求设计等待研发人员确认，开发和返修等待开发人员确认；actionToken 不能替代人员鉴权。
