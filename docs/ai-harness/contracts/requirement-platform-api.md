...
安装脚本和 skill 内容端点不得内置人员明文 Key，不得自动调用 reqflow MCP tool；脚本执行后只提示用户重启或刷新目标客户端。Codex 脚本可直接写 `~/.codex/config.toml`，并同步覆盖 Codex 实际可能加载的全局 skill 目录；Claude Code 和 CodeBuddy 优先调用各自 CLI 写入配置，Claude Code 同步覆盖 `~/.claude/skills`；OpenCode 必须写入或合并 `~/.config/opencode/opencode.json`（存在 `opencode.jsonc` 且无法自动解析时输出片段），CodeBuddy CLI 不可用时按官方优先级写入或合并 `~/.codebuddy/.mcp.json`、`~/.codebuddy/mcp.json` 或 `~/.codebuddy.json`；Trae、Qoder 按官方设置页 JSON 导入路径输出 `mcpServers` 片段并列入 `Manual MCP import required`。
